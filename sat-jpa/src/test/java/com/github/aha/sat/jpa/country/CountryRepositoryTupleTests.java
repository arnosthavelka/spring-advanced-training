...
class CountryRepositoryTupleTests {
	...
	class SimpleTupleUsage {
		...
		List<Tuple> countCitiesInCountriesLike(@NonNull String countryName) {
			return new JPAQuery<>(em).select(city.country.id.as("countryId"), city.country.name, city.country.count())
				.from(city)
		...
		void referenceByAttribute() {
			...
			assertThat(result).hasSize(4).allSatisfy(t -> {
		...
		void referrenceByIndexAndType() {
			...
			assertThat(result).hasSize(1).first().satisfies(t -> {
				assertThat(t.size()).isPositive();
	...
	class Paging {
		...
		Page<Tuple> findAllTuplesSortedBy(Pageable pageable, OrderSpecifier<?>... o) {
			var query = new JPAQuery<>(em).select(city.country.id, city.country.name, city.id, city.name)
				.from(city)
				.orderBy(o);
		...
		void unpagedResult() {
			...
			assertThat(result).hasSize(15).first().satisfies(t -> {
				assertThat(t.size()).isPositive();
		...
		void pagedResultSortedByCountry() {
			...
			assertThat(result).hasSize(pageSize).first().satisfies(t -> {
				assertThat(t.size()).isPositive();
				assertThat(t.get(city.country.id)).isPositive();
				assertThat(t.get(city.country.name)).isEqualTo(AUSTRALIA);
				assertThat(t.get(city.id)).isPositive();
